...
if len(argv) >= 2 and len(argv) <= 4:
    ...
    if argv[1] == "-h":
        ...
        flaga = 1
        print(f"Flag 1 is: {argv[1]}")
    else:
        opt1 = 1
        print(f"First Option is: {argv[1]}")
...
# there can be 3 flags, 3 options and n number of files. How to see those args
# One idea is to check if an argument exists by seeing if it is None or has value
# Trying to access argv[2] itself will raise index error, so I can enclose
...
try:
    ...
    print_help(msg="arg 2 is not given")
...
try:
    ...
    print_help(msg="arg 3 is not given")
